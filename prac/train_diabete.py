...
import prac.joint_transforms
from prac.config import cod_training_root
from prac.config import backbone_path
from prac.datasets import ImageFolder
from prac.misc import AvgMeter, check_mkdir
from prac.PFNet import PFNet
...
args = {
    ...
    'snapshot': '',
    'scale': 416,
    'save_point': [1, 3, 5, 7, 10, 13, 15, 17, 20, 25, 30, 35, 40],
    'poly_train': True,
    'optimizer': 'SGD',
}
...
# Transform Data.
joint_transform = prac.joint_transforms.Compose([
    prac.joint_transforms.RandomHorizontallyFlip(),
    prac.joint_transforms.Resize((args['scale'], args['scale']))
])
...
# loss function
structure_loss = prac.loss.structure_loss().cuda()
bce_loss = nn.BCEWithLogitsLoss().cuda()
iou_loss = prac.loss.IOU().cuda()
...
def main():
    ...
    net = PFNet(backbone_path).cuda().train()
